...
clustername='SpectralClustering'
clusterk=10
ans = SpectralClustering(n_clusters=clusterk, random_state=0).fit(clusterdata).predict(clusterdata)
def storFile(data,fileName):  
    ...
    with open(fileName,'w',newline ='') as f:  
        ...
        for i in data:  
            mywrite.writerow(i)  
storFile(ans,'cluster/'+clustername+str(clusterk)+'.csv')       

cm = mpl.colors.ListedColormap(list("rgbmyc")) 
plt.figure(figsize=(20,20),dpi=160)
plt.scatter(clusterdata[:, 0], clusterdata[:, 1], c=ans, s=30, cmap=cm, edgecolors='none')  
...
plt.title(clustername+str(clusterk))  
plt.savefig('cluster/'+clustername+str(clusterk)+'.png') 
plt.show()
...
#        print (chosen,'lasvegas feature_window:',feature_window,' mse:',mse)  
#        
