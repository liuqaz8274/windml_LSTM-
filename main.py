...
#    pred=tf.matmul(output,w_out)+b_out 
    pred2=tf.matmul(final_states,w_out)
    pred2=tf.add(pred2,b_out)
    tf.nn.softmax(pred2,name="output")
    return pred2,final_states  

def train_lstm(batch_size=80,time_step=15,train_begin=0,train_end=half):  
    X=tf.placeholder(tf.float32, shape=[None,time_step,input_size],name='input')  
...
#        saver.save(sess, "model/test1.ckpt")
        #############
        builder = tf.saved_model.builder.SavedModelBuilder('model/model4')
        builder.add_meta_graph_and_variables(sess, ['test'])
        builder.save()
        ###############
#        saver.restore(sess, "model/test1.ckpt")   
...
#            for j in range(len(test_x[step])):
#                fo = open('model/traininput.csv','a', newline='')
#                csv_write = csv.writer(fo,dialect='excel')
#                csv_write.writerow(test_x[step][j])
#                fo.close()
...
#            fo = open('model/testy.csv','a', newline='')
#            csv_write = csv.writer(fo,dialect='excel')
#            csv_write.writerow(test_y[j])
#            fo.close()
#        print(len(test_x))
#        print()
...
#        csv_write.writerow(res)
        print (' feature_window:',feature_window,' mse:',mse,'scalermin ',scaler_for_y.min_,'scalerscale ',scaler_for_y.scale_) ;   
